...
9. [Dev Containerとは（VS CodeでDocker環境を使う）](#9-dev-containerとはvs-codeでdocker環境を使う)
...
| コマンド | タイミング | コンテナの状態 |
|---------|-----------|-------------|
| `docker run -it イメージ bash` | コンテナを新規作成して入る | 新しく作られる |
| `docker exec -it コンテナ名 bash` | すでに起動中のコンテナに入る | 起動中である必要がある |
...
# 必要なライブラリをインストール
RUN pip install pandas numpy matplotlib jupyterlab
...
# ビルドしたイメージからコンテナを起動
docker run -p 8888:8888 my-python-env
```

> **今回のハンズオンでは Dockerfile を自分で書かず、既存の公式イメージ（Anaconda）を直接使います。** Dockerfileは「既存イメージをカスタマイズしたいとき」に使います。
...
## 9. Dev Containerとは（VS CodeでDocker環境を使う）
...
flowchart TB
    ...
    subgraph CON["📦 Dockerコンテナ（Dev Container）"]
        RUNTIME["Python / Node.js など\n実行環境"]
        EXTS["VS Code拡張機能\n（コンテナ側にインストール）"]
        MNTFILES["📁 マウントされた\nプロジェクトフォルダ"]
    end

    VSC -->|"devcontainer.jsonを読み込み\nコンテナを自動起動"| CON
    FILES <-->|"ボリュームマウント\nファイルが同期"| MNTFILES
    DCJSON --> VSC
...
`devcontainer.json` は Dev Containerの設定を記述するファイルです。

#### パターン① 既存のDockerイメージを使う（シンプル）
...
#### パターン② docker-compose.yml と連携する（応用）
...
| `dockerComposeFile` | 使用するdocker-composeファイル | `"docker-compose.yml"` |
| `customizations.vscode.extensions` | コンテナ内にインストールするVS Code拡張機能 | Pythonプラグインなど |
| `postCreateCommand` | コンテナ作成後に実行するコマンド | `"pip install -r requirements.txt"` |
| `forwardPorts` | ホストに転送するポート | `[8000, 8888]` |
| `remoteUser` | コンテナ内で使用するユーザー | `"vscode"` |

---

### Dev Containerを起動する手順
...
② 左下の「><」アイコンをクリック
   → または .devcontainer フォルダが検知されると右下に通知が出る

③「コンテナで再度開く（Reopen in Container）」を選択

④ 初回はコンテナのビルドが走る（数分かかる場合あり）

⑤ VS Codeの左下に「Dev Container: Python Dev Container」と表示されれば起動完了

⑥ ターミナルを開くとコンテナ内のシェルが使える
```
